...
Aperture-0 starts with an executable Phase 0 reference fixture derived from the v0.4 specification. Phase 0 is not the v0.4 MVP. It represents World A and World B, independent fixture clocks, a predefined causal graph, a deterministic four-state sequence, the specification's predefined demo metric values where supplied, a known-domain aperture opening/closing sequence, OPEN-only payload scramble/recovery, and verification of the exact preregistered fixture against a fixed terminal-hash trust anchor.
...
2. Every visual change is derived from a stored reference-fixture snapshot.
...
5. Fixture-integrity verification accepts only the exact trusted experiment identity, four-state order, record length, hash chain, manifest, and fixed terminal hash; coordinated truncation or resealing fails closed. It does not recompute model state and is not deterministic replay.
...
- The web UI can step, play, pause, and reset through the reference-fixture sequence.
